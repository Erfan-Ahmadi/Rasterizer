...
private:
    ...
    ID3D12Resource * depth_array[FrameQueueLength] = {};
    ...
    void Init_Rasterization () {
        ...
        // DepthArray Allocation
        {
            uint32_t depth_array_bytes = window_width * window_height * sizeof(int);
            D3D12_RESOURCE_DESC     resource_desc   = GetBufferResourceDesc(depth_array_bytes);
            resource_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        ...
        // Rasterizer Pass
        {
            ...
            // Create Root Signature
            {
                D3D12_DESCRIPTOR_RANGE1 descriptor_ranges[4] = {};
                descriptor_ranges[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;
                descriptor_ranges[0].NumDescriptors = 1;
                // : register(u0, space1)
                descriptor_ranges[0].BaseShaderRegister = 0;
                ...
                descriptor_ranges[3].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
                descriptor_ranges[3].NumDescriptors = 1;
                // : register(t1, space1)
                descriptor_ranges[3].BaseShaderRegister = 1;
                descriptor_ranges[3].RegisterSpace = 1;
                descriptor_ranges[3].Flags = D3D12_DESCRIPTOR_RANGE_FLAG_NONE;
                descriptor_ranges[3].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

                constexpr uint32_t NumParameters = 2;
                D3D12_ROOT_PARAMETER1 parameters[NumParameters] = {};
                parameters[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
                parameters[0].DescriptorTable.NumDescriptorRanges = 4;
                parameters[0].DescriptorTable.pDescriptorRanges = descriptor_ranges;
            ...
            // Create UAV+UAV+SRV+SRV
            {
                for(uint32_t i = 0; i < FrameQueueLength; ++i) {
                    ...
                    uav_desc.Buffer.Flags = D3D12_BUFFER_UAV_FLAG_NONE;
                    device->CreateUnorderedAccessView(depth_array[i], nullptr, &uav_desc, current_cpu_handle);
                    current_cpu_handle.ptr += device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
                    current_gpu_handle.ptr += device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
void Demo_003_RasterizerCompute::OnRender() {
    ...
    current_cmd_list->Reset(direct_cmd_allocator, nullptr);
    {
        ...
        if(true == use_software_rasterizer) {
            ...
            // COPY FROM FRAMEBUFFER to SWAPCHAIN RENDERTARGET
